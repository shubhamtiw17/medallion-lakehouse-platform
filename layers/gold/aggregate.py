...
def build_gold_diabetes():
    ...
    # Aggregate: outcomes by age group
    df["age_group"] = pd.cut(
        df["Age"],
        bins=[0, 30, 45, 60, 100],
    ...
    gold["diabetes_rate_pct"] = (
        (gold["diabetic_count"] / gold["total_patients"]) * 100
    ).round(2)
...
def build_gold_heart():
    ...
    # Aggregate: heart disease by age group and sex
    df["age_group"] = pd.cut(
        df["age"],
        bins=[0, 40, 55, 70, 100],
    ...
    gold["disease_rate_pct"]  = (
        (gold["disease_count"] / gold["total_patients"]) * 100
    ).round(2)
...
if __name__ == "__main__":
    ...
    for name in ["covid_cases", "diabetes", "heart_disease"]:
        con = duckdb.connect()
        count = con.execute(
            f"SELECT COUNT(*) FROM read_parquet('layers/gold/{name}/data.parquet')"
        ).fetchone()[0]
        sample = con.execute(
            f"SELECT * FROM read_parquet('layers/gold/{name}/data.parquet') LIMIT 2"
        ).df()
